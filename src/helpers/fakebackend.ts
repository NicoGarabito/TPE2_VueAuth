    ...
    window.fetch = function (url, opts: any): Promise<Response> {
        return new Promise((resolve, reject) => {
            ...
            function refreshTokens() {
                ...
                const user = users.find(x => x.refreshTokens.includes(refreshTokens));
                if (!user) return unauthorized();

                // Reemplazar refresh token viejo por uno nuevo y guardar
                user.refreshTokens = user.refreshTokens.filter(x => x !== refreshTokens);
                user.refreshTokens.push(generaterefreshTokens());
                localStorage.setItem(usersKey, JSON.stringify(users));
            ...
            function revokeToken() {
                ...
                const refreshTokens = getrefreshTokens();
                const _user = users.find(x => x.refreshTokens.includes(refreshTokens));

                // Revocar token y guardar en almacenamiento local
                if(_user !==  undefined) {
                    _user.refreshTokens = _user.refreshTokens.filter(x => x !== refreshTokens);
                    localStorage.setItem(usersKey, JSON.stringify(users));
                }
            ...
            function generaterefreshTokens(): string {
